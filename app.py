...
def bulk_delete_worker(service, senders_list):
    """Safely processes multiple senders sequentially on a single background thread."""
    for sender in senders_list:
        try:
            # Re-use your perfectly working page-by-page deletion logic safely
            delete_existing_emails(service, sender)
        except Exception as e:
            # Fail silently in the background so the app never crashes
            pass
  ...
  # 3. Dynamic Table Display with Multi-Select Checkboxes
    ...
    if selected_senders:
        ...
        with st.container(border=True):
            ...
            # 1. SAFE BULK DELETE ACTION
            if b_del.button("🗑️ Bulk Delete", use_container_width=True, type="primary"):
                # Extract just the email addresses to pass to the worker
                emails_to_delete = [sender for sender, _ in selected_senders]
                
                # Instantly update metrics and session states on the MAIN thread
                for sender, _ in selected_senders:
                    st.session_state.actioned_senders.add(sender)
                    st.session_state.total_size -= st.session_state.sender_sizes.get(sender, 0)
                
                # Spawn ONE single thread to process the entire batch sequentially
                bulk_thread = threading.Thread(
                    target=bulk_delete_worker, 
                    args=(service, emails_to_delete)
                )
                bulk_thread.start()
                
                st.toast(f"🧹 Background cleanup started for {len(emails_to_delete)} senders!")
                time.sleep(1)
                st.rerun()
