...
if 'user_id_hash' not in st.session_state: # Added to persist the ID
    st.session_state.user_id_hash = None

conn = st.connection("gsheets", type=GSheetsConnection)

# --- 2. LOGGING ENGINE (Moved Up) ---
...
def create_future_filter(service, sender_email, user_id_hash): # Added hash arg
    filter_rule = {
...
def confirm_future_delete(service, sender_email, user_id_hash): # Added hash arg
...
if st.session_state.leaderboard:
    ...
    with st.container(border=True):
        ...
        for rank, (sender, estimate) in enumerate(top_k, 1):
            ...
            # Fetch real count
            real_results = service.users().messages().list(userId='me', q=f"from:{sender} is:unread in:inbox", maxResults=500).execute()
